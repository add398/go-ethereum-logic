...
func (t *Trie) Get(key []byte) ([]byte, []int) {
	res, err, num := t.TryGet(key)
	if err != nil {
...
func (t *Trie) TryGet(key []byte) ([]byte, error, []int) {
	//fmt.Println(keybytesToHex(key))
	value, newroot, didResolve, err, num := t.tryGet(t.root, keybytesToHex(key), 0)
...
func (t *Trie) tryGet(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error, num []int) {
	switch n := (origNode).(type) {
	case nil:
		return nil, nil, false, nil, []int{0, 0}
	case valueNode:
		return n, n, false, nil, []int{0, 1}
	case *shortNode:
		if len(key)-pos < len(n.Key) || !bytes.Equal(n.Key, key[pos:pos+len(n.Key)]) {
			// key not found in trie
			return nil, n, false, nil, []int{0, 0}
		}
		value, newnode, didResolve, err, num = t.tryGet(n.Val, key, pos+len(n.Key))
		if err == nil && didResolve {
			n = n.copy()
			n.Val = newnode
		}
		num[1] = num[1] + 1
		return value, n, didResolve, err, num
	case *fullNode:
		value, newnode, didResolve, err, num = t.tryGet(n.Children[key[pos]], key, pos+1)
		if err == nil && didResolve {
			n = n.copy()
	...
	case hashNode:
		...
		if err != nil {
			return nil, n, true, err, []int{0, 0}
		}
		value, newnode, _, err, num := t.tryGet(child, key, pos)
		return value, newnode, true, err, num
	default:
